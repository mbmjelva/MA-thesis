...
for(var in vars){
  ...
  for(i in 1:length(var_quantiles)){
    ...
    if(i == 1){
      res <- average_treatment_effect(cf_neg, subset = train_speineg[[var]] <= qtl, target.sample = "overlap")
    } else if (qtl == qtl_l) {
      res <- average_treatment_effect(cf_neg, subset = train_speineg[[var]] <= qtl, target.sample = "overlap")
    } else {
      res <- average_treatment_effect(cf_neg, subset = (train_speineg[[var]] <= qtl) & (train_speineg[[var]] > qtl_l), target.sample = "overlap")
    }
...
save(cates_neg, file = "./R-script, analysis/Models/cates_neg_with_target.rds")
...
for(var in vars){
  var_quantiles <- quantile(train_speipos[[var]], probs = seq(0, 1, 0.25)) # Set to vary between lowest and highest values of the variables
  for(i in 1:length(var_quantiles)){
    qtl <- var_quantiles[i]
    qtl_l <- var_quantiles[i-1]
    if(i == 1){
      res <- average_treatment_effect(cf_pos, subset = train_speipos[[var]] <= qtl, target.sample = "overlap")
    } else if (qtl == qtl_l) {
      res <- average_treatment_effect(cf_pos, subset = train_speipos[[var]] <= qtl, target.sample = "overlap")
    } else {
      res <- average_treatment_effect(cf_pos, subset = (train_speipos[[var]] <= qtl) & (train_speipos[[var]] > qtl_l), target.sample = "overlap")
    }
...
save(cates_pos, file = "./R-script, analysis/Models/cates_pos_with_target.rds")
