### Estimation of CATEs ###
# Lagged, spei binary, cluster gwno
...
test_speineg <- dplyr::select(test_speineg, -conflict, -spei3_neg, -lon, -lat, -gwno, -lag_conflict, -non_state_conflict, -excluded) # Must remove the cluster-variable and treatment and outcome
train_speineg <- dplyr::select(train_speineg, -conflict, -spei3_neg, -lon, -lat, -gwno, -lag_conflict, -non_state_conflict, excluded) # Prøver med å fjerne conflict
...
for(var in vars){
  var_quantiles <- quantile(train_speineg[[var]], probs = seq(0, 1))
  for(i in 1:length(var_quantiles)){
    qtl <- var_quantiles[i]
    qtl_l <- var_quantiles[i-1]
    if(i == 1){
      res <- average_treatment_effect(cf_neg_dich_clgwno, subset = train_speineg[[var]] <= qtl, target.sample = "overlap")
    } else if (qtl == qtl_l) {
      res <- average_treatment_effect(cf_neg_dich_clgwno, subset = train_speineg[[var]] <= qtl, target.sample = "overlap")
    } else {
      res <- average_treatment_effect(cf_neg_dich_clgwno, subset = (train_speineg[[var]] <= qtl) & (train_speineg[[var]] > qtl_l), target.sample = "overlap")
    }
    cates_neg <- bind_rows(cates_neg, tibble("variable" = var, "quantile" = qtl, "ape" = res[1], "ape_std" = res[2]))
  }
}

quantile(train_speineg$libdem, probs = seq(0.05, 0.95, 0.25))
a <- average_treatment_effect(cf_neg_dich_clgwno, subset = train_speineg$libdem > 0.8, target.sample = "overlap") # Kan eventuelt gå gjennom hvert skritt her for å finne ut hvordan det varierer.
a
...
save(cates_neg, file = "./R-script, analysis/Models/cates_neg_speidich_clgwno_with_target.rds")
...
for(var in vars){
  ...
  for(i in 1:length(var_quantiles)){
    ...
    qtl_l <- var_quantiles[i-1]
    if(i == 1){
      res <- average_treatment_effect(cf_pos_dich_clgwno, subset = train_speipos[[var]] <= qtl, target.sample = "overlap")
    } else if (qtl == qtl_l) {
      res <- average_treatment_effect(cf_pos_dich_clgwno, subset = train_speipos[[var]] <= qtl, target.sample = "overlap")
    } else {
      res <- average_treatment_effect(cf_pos_dich_clgwno, subset = (train_speipos[[var]] <= qtl) & (train_speipos[[var]] > qtl_l), target.sample = "overlap")
    }
...
save(cates_pos, file = "./R-script, analysis/Models/cates_pos_speidich_clgwno_with_target.rds")
