...
# Plots with variables with heterogeneity ---------------------------------
catt_neg %>% filter(variable %in% c("bdist3", "pop", "shdi", "temp", "ttime_mean")) %>% 
  ...
  geom_errorbar() + 
  theme_light() +
  scale_x_continuous(limits = c(-0.1, 0.21), breaks = seq(-0.1, 0.21, 0.05)) +
  theme(
...
ggsave("./Figurer/catt_neg, speidich, only conflict, vars with het.png")


catt_pos %>% filter(variable %in% c("agri_ih", "bdist3", "capdist", "empl_agr", "excluded", "gdp")) %>% 
  ggplot(aes(y = as.factor(round(quantile, digits = 1)), x = catt, xmin = cattmin, xmax = cattmax, color = significant)) + 
  facet_grid(vars(variable), scales = "free") +
  geom_point() + 
  geom_errorbar() + 
...
ggsave("./Figurer/catt_pos, speidich, only conflict, vars with het 1.png")

catt_pos %>% filter(variable %in% c("global_ind", "irrig_sum", "libdem", "pop",  "temp", "ttime_mean")) %>% 
  ggplot(aes(y = as.factor(round(quantile, digits = 1)), x = catt, xmin = cattmin, xmax = cattmax, color = significant)) + 
  facet_grid(vars(variable), scales = "free") +
  geom_point() + 
  geom_errorbar() + 
...
ggsave("./Figurer/catt_pos, speidich, only conflict, vars with het 2.png")

# Plots with all variables (for appendix) ---------------------------------
