## CATT, speidich, all countries ##

library(tidyverse)

# Spei neg
load(file = "./R-script, analysis/Models/catt_neg_speidich.rds")
...
catt_neg %>% filter(variable %in% c("libdem", "temp", "capdist", "bdist3", "ttime_mean", "global_ind", "pop")) %>% 
  ...
  geom_errorbar() + 
  theme_light() +
  scale_x_continuous(limits = c(-0.04, 0.11), breaks = seq(-0.04, 0.11, 0.03)) +
  theme(
...
ggsave("./Figurer/catt_neg, speidich, all countries, 1.png", height = 15, width = 11)
...
ggsave("./Figurer/catt_neg, speidich, all countries, 2.png", height = 15, width = 11)


# Spei pos
load(file = "./R-script, analysis/Models/catt_pos_speidich.rds")
...
ggsave("./Figurer/catt_pos, speidich, all countries, 1.png",  height = 15, width = 11)
...
ggsave("./Figurer/catt_pos, speidich, all countries, 2.png", height = 15, width = 11)
