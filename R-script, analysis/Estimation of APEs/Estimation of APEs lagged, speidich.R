...
for(var in vars){
  ...
  for(i in 1:length(var_quantiles)){
    ...
    if(i == 1){
      ...
      res <- average_partial_effect(cf_neg_speidich, subset = train_speineg[[var]] <= qtl)
      ...
      res <- average_partial_effect(cf_neg_speidich, subset = (train_speineg[[var]] <= qtl) & (train_speineg[[var]] > qtl_l))
...
save(apes_neg, file = "./R-script, analysis/Models/apes_neg_speidich.rds")
...
for(var in vars){
  ...
  for(i in 1:length(var_quantiles)){
    ...
    qtl_l <- var_quantiles[i-1]
    if(i == 1){
      res <- average_partial_effect(cf_pos_speidich, subset = train_speipos[[var]] <= qtl)
    } else if (qtl == qtl_l) {
      res <- average_partial_effect(cf_pos_speidich, subset = train_speipos[[var]] <= qtl)
    } else {
      res <- average_partial_effect(cf_pos_speidich, subset = (train_speipos[[var]] <= qtl) & (train_speipos[[var]] > qtl_l))
    }
...
save(apes_pos, file = "./R-script, analysis/Models/apes_pos_speidich.rds")
