## Plotte effekter with lagged variables, cluster gwno ##
...
# Må endre metoden slik at den tar med cluster når lager apes. cf_neg hetetr lagged foreløpig fordi ble overskrevet.

library(tidyverse)
library(grf)

load(file = "./R-script, analysis/Models/cf_neg_cluster_gwno.rds")
...
for(var in vars){
  ...
  for(i in 1:length(var_quantiles)){
    ...
    if(i == 1){
      ...
      res <- average_partial_effect(cf_neg_gwno, subset = train_speineg[[var]] <= qtl)
      ...
      res <- average_partial_effect(cf_neg_gwno, subset = (train_speineg[[var]] <= qtl) & (train_speineg[[var]] > qtl_l))
...
save(apes_neg, file = "./R-script, analysis/Models/apes_neg_cluster_gwno.rds")
# Hvorfor er alle positive her? Hva betyr dette vs det vi får ut fra predict? 

# Positive, 25 quantiler
load(file = "./R-script, analysis/Models/cf_pos_cluster_gwno.rds")
...
save(apes_pos, file = "./R-script, analysis/Models/apes_pos_cluster_gwno.rds")
